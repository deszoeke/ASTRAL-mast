function hed = read_hed_pitch(dfl,ddd,hhh)
%{
reads heading/pitch file from PSD Hemisphere system

outputs:
1   jd_ref      10 Hz timestamp
2   hed         heading, deg
3   pit         pitch angle, deg
...
% reference timestamp
disp(['Reading hed pitch file for hour ',int2str(hhh)]);
start = (ddd+hhh/24);
...
if exist(dfl,'file')==2
    ...
    while feof(flist)==0	% read entire file into cell array
        try
            ...
            % 0000106 $PSAT,HPR,000002.00,184.19,0.53,,N*3D
            temp = textscan(flist,'%2f%2f%3f %*5c %*3c %f %f %f %*[^\n]','delimiter',', ','headerlines',1,'emptyvalue', NaN);
    ...
    pitch = replace_NaN_nearest_neighbor(stx(6,:));
    ...
    hed(:,2) = interp1(jd_hed,heading,tref);
    hed(:,3) = interp1(jd_hed,pitch,tref);
